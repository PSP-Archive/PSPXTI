...
psp_display_screen_kbd_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_KBD_ITEM; menu_id++) 
  {
        ...
        (menu_id <= MENU_KBD_JOY_RIGHT)) 
    {
      ...
      if (menu_kbd_selected == KBD_NORMAL_MAPPING  ) ti92_key = loc_kbd_mapping[kbd_id];
      else
      if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) ti92_key = loc_kbd_mapping_L[kbd_id];
      else
      if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) ti92_key = loc_kbd_mapping_R[kbd_id];

      if ((ti92_key >= 0) && (ti92_key < TI92_LAST_BUTTON)) {
        strcpy(buffer, ti92_buttons_info[ti92_key].name);
      } else 
      if (ti92_key == KBD_UNASSIGNED) {
        sprintf(buffer, "UNASSIGNED");
      } else
      if (ti92_key == KBD_LTRIGGER_MAPPING) {
        sprintf(buffer, "L MAPPING");
      } else
      if (ti92_key == KBD_RTRIGGER_MAPPING) {
        sprintf(buffer, "R MAPPING");
      } else {
        sprintf(buffer, "KEY %d", ti92_key);
      }
...
# ifdef USE_PSP_IRKEYB
    ...
    if (psp_irkeyb != PSP_IRKEYB_EMPTY) {
      ...
      if (psp_irkeyb == 0x8) {
        ti92_key = TI92_BUTTON_DEL;
      } else
      if (psp_irkeyb == 0x9) {
        ti92_key = TI92_BUTTON_F1;
      } else
      if (psp_irkeyb == 0xd) {
      ...
      if (ti92_key != -1) {
            ...
            (cur_menu_id <= MENU_KBD_JOY_RIGHT)) 
        {
          ...
          if (menu_kbd_selected == -1) loc_kbd_mapping[kbd_id] = ti92_key;
          else
          if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) loc_kbd_mapping_L[kbd_id] = ti92_key;
          else
          if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) loc_kbd_mapping_R[kbd_id] = ti92_key;
        }
      }
    }
# endif
